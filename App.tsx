...
const App: React.FC = () => {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      if (kbRes.data?.content) {
        setKb(kbRes.data.content);
      }
  ...
  const syncPlan = useCallback(async (dataToSaveOverride?: any) => {
  ...
  const handleNavigate = (id: number) => setActivePillarId(id);
  ...
  const handleAddAction = (pillarId: number, newAction: any) => {
    ...
    setPillars(prev => {
      ...
      syncPlan(updated.map(up => ({ id: up.id, actions: up.actions })));
      return updated;
    });
  ...
  const handleAgentAction = (type: 'email' | 'calendar', details: string) => {
    setNotification({ message: details, type: 'action' });
        ...
        <div className="bg-gray-100 px-6 py-2 flex justify-between items-center border-b border-gray-200">
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2 text-[10px] font-black uppercase tracking-widest">
              {syncStatus === 'saved' && <><CloudCheck className="w-3 h-3 text-green-500" /> <span className="text-green-600">Sync Active</span></>}
              {syncStatus === 'no-schema' && <><AlertTriangle className="w-3 h-3 text-red-500" /> <span className="text-red-600">DB Offline</span></>}
            </div>
